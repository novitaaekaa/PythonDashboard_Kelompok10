...
if data_provinsi is not None and not data_provinsi.empty:
    ...
    # Tabs untuk berbagai view (DIPERBAIKI)
            ...
            if lat_col and lon_col and poverty_col:
                ...
                try:
                    ...
                    if not map_data.empty:
                        ...
                        # Update traces untuk hover yang lebih baik dan menghilangkan "undefined"
                    ...
                    with st.expander("🔍 Debug Info"):
                        ...
                        st.write(f"- Latitude: {lat_col}")
                        st.write(f"- Longitude: {lon_col}")  
                        st.write(f"- Kemiskinan: {poverty_col}")
                        
                        st.write("**Sample data mentah:**")
                        if lat_col and lon_col and poverty_col:
                            cols_to_show = [col for col in [lat_col, lon_col, poverty_col] if col in filtered_data.columns]
                            if cols_to_show:
                ...
                with st.expander("📋 Informasi Kolom yang Dibutuhkan"):
                    st.write("**Kolom yang harus ada dalam data:**")
                    st.write("✅ **Latitude**: Kolom dengan nama yang mengandung 'LATITUDE' atau 'LAT'")
                    st.write("✅ **Longitude**: Kolom dengan nama yang mengandung 'LONGITUDE', 'LON', atau 'LONG'")
                    st.write("✅ **Kemiskinan**: Kolom dengan nama yang mengandung 'PENDUDUK_MISKIN', 'MISKIN', atau 'KEMISKINAN'")
                    
                    st.write(f"\n**Kolom yang tersedia dalam data:** {', '.join(list(data_provinsi.columns))}")
                    
                    st.write(f"\n**Status deteksi:**")
                    st.write(f"- Latitude: {'✅ Ditemukan' if lat_col else '❌ Tidak ditemukan'} ({lat_col})")
                    st.write(f"- Longitude: {'✅ Ditemukan' if lon_col else '❌ Tidak ditemukan'} ({lon_col})")
                    st.write(f"- Kemiskinan: {'✅ Ditemukan' if poverty_col else '❌ Tidak ditemukan'} ({poverty_col})")
    ...
    with tab2:
        ...
        # TPT Bar chart dengan pengecekan kolom
        if "TPT (%)" in filtered_data.columns:
